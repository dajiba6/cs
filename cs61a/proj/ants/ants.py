...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def near_place(place, beehive, dist):
            if place == beehive:
                return None
            if place.bees != [] and self.min_range <= dist <= self.max_range:
                return bee_selector(place.bees)
            dist += 1
            return near_place(place.entrance, beehive, dist)

        return near_place(self.place, beehive, dist)
